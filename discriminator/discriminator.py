...
class SpamDiscriminator(nn.Module):
    ...
    def _generate_char_vectors(
        self, chinese_characters, w2v_vectors, sim_mat, texts, chinese_characters_count, threshold=None
    ):
        """
        生成字符向量
        """
    ...
    def train_batch(self, batch_texts, batch_labels, optimizer):
        """
        训练单个batch的判别器
        Args:
            batch_texts: list[str], 当前batch的文本列表
            batch_labels: list[str], 当前batch的标签列表("spam"/"normal")
            optimizer: 优化器对象
        Returns:
            batch_loss: float, 当前batch的损失值
            batch_acc: float, 当前batch的准确率
        """
        print("Discriminator train_step")
        self.train()
        batch_texts, batch_labels_str = self.create_contrastive_pairs(batch_texts, batch_labels, len(batch_texts))
        ...
        # 前向传播
        predictions = self.classifier(batch_sentence_tensors).squeeze(-1)
        ...
        # 组合总损失
        total_loss = (1 - self.contrastive_weight) * classification_loss + self.contrastive_weight * contrastive_loss
    ...
    def evaluate(self, texts, labels):
        ...
        labels_tensor = torch.tensor(
            [1.0 if label == "spam" else 0.0 for label in labels], dtype=torch.float32, device=self.device
        )
        with torch.no_grad():
    ...
    def _tokenize_and_remove_stopwords(self, texts, stopwords_file=None):
        ...
        for text in texts:
            cleaned_text = "".join(
                [char for char in text if char not in stopwords and re.search("[\u4e00-\u9fa5]", char)]
            )
